...
void SampleClk_Start(void) 
{
    ...
    SampleClk_CLKEN |= SampleClk_CLKEN_MASK;
	SampleClk_CLKSTBY |= SampleClk_CLKSTBY_MASK;
}
...
void SampleClk_Stop(void) 
{
    ...
    SampleClk_CLKEN &= (uint8)(~SampleClk_CLKEN_MASK);
	SampleClk_CLKSTBY &= (uint8)(~SampleClk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: SampleClk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((SampleClk_CLKEN & SampleClk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = SampleClk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = SampleClk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* SampleClk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(SampleClk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(SampleClk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: SampleClk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SampleClk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = SampleClk_GetSourceRegister();
    uint16 oldDivider = SampleClk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = SampleClk_CLKEN & SampleClk_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((SampleClk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: SampleClk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(SampleClk_DIV_PTR);
}


/*******************************************************************************
* Function Name: SampleClk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    SampleClk_MOD_SRC |= modeBitMask & (uint8)SampleClk_MODE_MASK;
}


/*******************************************************************************
* Function Name: SampleClk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    SampleClk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(SampleClk_MODE_MASK));
}


/*******************************************************************************
* Function Name: SampleClk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return SampleClk_MOD_SRC & (uint8)(SampleClk_MODE_MASK);
}


/*******************************************************************************
* Function Name: SampleClk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = SampleClk_GetDividerRegister();
    uint8 oldSrc = SampleClk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        SampleClk_MOD_SRC |= CYCLK_SSS;
        SampleClk_MOD_SRC =
            (SampleClk_MOD_SRC & (uint8)(~SampleClk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: SampleClk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return SampleClk_MOD_SRC & SampleClk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: SampleClk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    SampleClk_PHASE = clkPhase & SampleClk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: SampleClk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return SampleClk_PHASE & SampleClk_PHASE_MASK;
}
