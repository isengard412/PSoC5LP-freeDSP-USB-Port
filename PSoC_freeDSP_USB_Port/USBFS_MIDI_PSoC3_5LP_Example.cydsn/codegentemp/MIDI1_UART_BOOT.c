         ...
         
{
    ...
    while(bufIndex < size)
    {
        MIDI1_UART_PutChar(pData[bufIndex]);
        bufIndex++;
    }
         ...
         
{
    ...
    *  If at least one byte is received within the time out interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        *  MIDI1_UART_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(MIDI1_UART_BL_CHK_DELAY_MS);
        }
    }
    ...
    while(MIDI1_UART_GetRxBufferSize() > 0u)
    {
        ...
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            ...
            *  If not wait for additional MIDI1_UART_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != MIDI1_UART_PACKET_EOP)
            {
                CyDelay(MIDI1_UART_WAIT_EOP_DELAY);
            }
        }
